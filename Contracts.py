...
if __name__ == '__main__':
    ...
    next_month = months[0] # 得到最近一个月

    # 获取2025年12月到期的上证50期权行日（同一月份的其它标的行权日都是相同的）
    expire_day = ak.option_sse_expire_day_sina(next_month, '50ETF')
    print(f'上证50ETF期权{next_month}合约行权日：{expire_day[0]}， 距到期日：{expire_day[1]}天')


    # 获取2025年12月到期的上证50（510050）所有认购期权合约
    df_contracts = ak.option_sse_codes_sina('看涨期权', next_month, '510050')
    # print(df_contracts)
    # 获取2025年12月到期的上证50（510050）所有认沽期权合约
    df_contracts = ak.option_sse_codes_sina('看跌期权', next_month, '510050')
    # print(df_contracts)
